...
export class TemplateStoreService {
  getTemplates(): Promise<ITemplate[]> {
    return this.getService()
      .then(s => {
        ...
        return s
          .queryCollectionNames([AccountDocumentCollectionId])
          .then(collections => {
            if (collections.length > 0) {
              const accountTemplates: ITemplate[] =
  ...
  createTemplate(template: ITemplate): Promise<ITemplate> {
    return this.getService().then(service =>
      service.createDocument(this.getCollectionId(), template)
    );
  }

  updateTemplate(template: ITemplate): Promise<ITemplate> {
    return this.getService().then(service =>
      service.updateDocument(this.getCollectionId(), template)
    );
  }

  deleteTemplate(template: ITemplate): Promise<void> {
    return this.getService().then(service =>
      service.deleteDocument(this.getCollectionId(), template.id)
    );
  }

  private getService(): Promise<ExtensionDataService> {
    return new Promise((resolve, reject) =>
      VSS.getService(VSS.ServiceIds.ExtensionData).then(resolve, reject)
    );
  }
